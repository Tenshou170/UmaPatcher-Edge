...
internal object SplitApkMerger {
    ...
    private fun mergeManifestElements(merged: ApkModule, bundle: ApkBundle) {
        ...
        for (splitModule in bundle.apkModuleList) {
            if (splitModule === merged) continue
            val splitManifest = try {
                splitModule.androidManifestBlock
            } catch (_: IOException) {
                continue
            } ?: continue
            val splitManifestElement = splitManifest.manifestElement ?: continue

            // Direct children of <manifest> (uses-permission, permission, uses-feature, uses-library...).
            for (splitChild in splitManifestElement.listElements()) {
                val tag = splitChild.name ?: continue
                if (tag == AndroidManifestBlock.TAG_application) continue
